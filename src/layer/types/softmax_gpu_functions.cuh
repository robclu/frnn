...
#ifndef _CURNN_SOFTMAX_FUNCTIONS_GPU_
#define _CURNN_SOFTMAX_FUNCITONS_GPU_
...
void softmaxUpadateWbaGpu( Tensor4<dType>& prev_wba, uint act_start, size_t N, 
    ...
    {
        floati  *prev_wba_d, *curr_wba_d;
        int      thread_id;
        // Allocate device memory for weights 
        i`f ( cudaMalloc( (void**)&prev_wba_d, N * sizeof( dType ) ) != cudaSuccess ) {
            curnn::err::allocError( error, stringify( prev_wba_d ) );
        }
        if ( cudaMalloc( (void**)&curr_wba_d, M * sizeof( dType ) ) != cudaSuccess ) {
            curnn::err::allocError( error, stringify( curr_wba_d ) );
        }
        
        // Copy to device memory 
        if ( cudaMemcpy( prev_wba_d,          &prev_wba( act_start, 0, thread_id, 0 ), 
                         N * sizeof( dType ), cudaMemcpyHostToDevice ) != cudaSuccess ) {
            curnn::err::copyError( error, stringify( prev_wba ) );
        }
